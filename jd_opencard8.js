...
17 3,11,19 * * * https://raw.githubusercontent.com/KingRan/JDJB/main/jd_opencard8.js

*/
const $ = new Env("婴乐家会员日");
...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            authorCodeList = [
                'BF366C96CC115C438DFFF010721E5EFC39FB9547AF09B17C0282335609174303444964BAA6654E6A6B1B7A78C3FC98905FC8FFF43FE734E7BC3DB412BC1E1375DDDA672BF446E2FCC0D1D6B4E52826D1',
                ...
                '03A67F758C6426FB19A32AE6B4428112D892E441F0B98F84F61CC116DF06C7EF49336DE54E26AA8F2834B248E6398CB7A755DF4FDAE585EC3E1ABE26F3DD3CFFC956D12974FF00A045D8E31A84FE84C18A8357DE96A1F617B8AC4D64BC24B689',
            ]
            $.bean = 0;
...
async function member_08() {
    ...
    if ($.token) {
        ...
        if ($.buyerNick) {
            ...
            await task('uniteOpenCardStats', {
                "actId": $.actId,
                "pushWay": 1,
                "missionType": "pv",
            })
            await task('loadUniteOpenCard', {
                "inviteNick": $.authorCode,
                "actId": $.actId
            })
            if ($.activityInfo) {
                if (!$.activityInfo.customer.isFocusAward) {
                ...
                if (!$.activityInfo.customer.isCartAward) {
                    await task('addCart', {
                        "actId": $.actId,
                        "missionType": "addCart",
                    })
                            ...
                            if (data.data.status === 200) {
                                switch (function_id) {
                                    ...
                                    case 'uniteOpenCardOne':
                                        ...
                                        $.needOpen = data.data.data.succ
                                        break;
                                    case 'loadUniteOpenCard':
                                        console.log(`  =>${data.data.data.isOpenCard}`)
                                        console.log(`  =>${data.data.data.msg}`)
...
function taskUrl(function_id, body) {
    return {
        url: `https://jinggengjcq-isv.isvjcloud.com/dm/front/openCard/${function_id}?open_id=&mix_nick=${$.buyerNick ? $.buyerNick : ""}&bizExtString=${$.authorCode ? Base64.toBase64(`shareNick:${$.authorCode}`) : ""}&user_id=${$.userId}`,
        headers: {
